...
<script>
let gpsStatus='idle';let watchId=null;
function updateGPS(force=false){
if(!navigator.geolocation){renderGPSUI('error','Browser Not Supported');return}
if(force){gpsStatus='locking';renderGPSUI('loading','Mencari Sinyal...')}
...
const el=document.getElementById('gps-status');const dot=document.getElementById('gps-dot');const txt=document.getElementById('gps-text');if(!el)return;
el.classList.remove('opacity-0','translate-y-2');el.classList.add('opacity-100','translate-y-0');
if(type==='success'){dot.className="h-2.5 w-2.5 rounded-full bg-green-500 shadow-[0_0_8px_#22c55e]";txt.className="text-[10px] md:text-xs font-black text-green-600 dark:text-green-400"}
else if(type==='error'){dot.className="h-2.5 w-2.5 rounded-full bg-red-500 animate-pulse";txt.className="text-[10px] md:text-xs font-bold text-red-500"}
else{dot.className="h-2.5 w-2.5 rounded-full bg-orange-400 animate-bounce";txt.className="text-[10px] md:text-xs font-medium text-orange-500"}
txt.innerText=text;
}
document.addEventListener('DOMContentLoaded',()=>updateGPS());
let scanner=null;let isActive=false;let lastScan=0;
async function processScan(code){
const now=Date.now();if(isActive||(now-lastScan<7000))return;isActive=true;lastScan=now;if(navigator.vibrate)navigator.vibrate([150,50,150]);
const overlay=document.getElementById('global-processing');const results=document.getElementById('scan-result');
overlay.classList.remove('hidden');if(results){results.innerText="QR Ditemukan! Menghubungkan ke Pusat...";results.classList.remove('hidden','bg-red-500','bg-green-600');results.classList.add('bg-orange-500')}
...
if(confirm("Koneksi terputus. Coba lagi?"))if(scanner)scanner.resume();
}
}
window.addEventListener('attendance-success',()=>{
document.getElementById('global-processing').classList.add('hidden');const results=document.getElementById('scan-result');
if(results){results.innerText="ABSENSI BERHASIL DISIMPAN!";results.classList.replace('bg-orange-500','bg-green-600');setTimeout(()=>results.classList.add('hidden'),7000)}
setTimeout(()=>{isActive=false;if(scanner)scanner.resume()},7000);
});
...
scanner=new Html5QrcodeScanner("reader",{fps:30,qrbox:{width:250,height:250},aspectRatio:1.0,videoConstraints:{facingMode:"environment",focusMode:"continuous",width:{ideal:1280},height:{ideal:720}}},false);
